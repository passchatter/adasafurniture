...
const DetailProduk = () => {
  ...
  const getDetail = async () =>{
    try{
      const response = await axios.get(`http://adasabalifurniture.com:8000/api/produks/detail/${id}/`)
  ...
  const getSimilar = async () => {
    try{
        const response = await axios.get(`http://adasabalifurniture.com:8000/api/produks/similar/${id}/`)
        return response.data
    }catch(error){
        console.log(error)
    }
    ...
    <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="mx-auto">
                <img className="border rounded-lg border-slate-200 w-full md:max-w-[600px] h-auto" src={`http://adasabalifurniture.com:8000${detail.image}`} alt="" />
              </div>
              ...
              <SwiperSlide>
               <Link to={`/produks/detail/${sml.id}`}>
                    <div className="" key={index}>
                        <img className='md:w-[350px] md:h-[320px] w-[200px] h-[220px]' src={`http://adasabalifurniture.com:8000${sml.image}`} alt="" />
                    </div>
               </Link>
               </SwiperSlide>
            )
