...
const BlogDetail = () => {
    ...
    useEffect(() => {
        const fetchBlogDetail = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/blog/detail/${id}/`);
                        ...
                        <ul className='md:max-w-4xl lg:max-w-5xl'>
                            {blogDetail.entry && blogDetail.entry.map(entry => (
                                <li key={entry.id} className="mb-6">
                                    <img src={`http://127.0.0.1:8000/${entry.image}`} alt={`Image ${entry.id}`} className="md:w-full md:h-[400px] xl:h-[500px]" />
